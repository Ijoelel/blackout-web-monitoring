...
function buildGeneratorHistory(g: GeneratorData, now: number) {
  ...
  if (g.load_kw != null) hist.load_kw = genHistory({ base: g.load_kw, jitter: 0.03, points: 60, min: 0, now })
  if (g.frequency_hz != null) hist.frequency_hz = genHistory({ base: g.frequency_hz, jitter: 0.001, points: 60, now })
  if (g.lube_oil_pressure_bar != null)
    hist.lube_oil_pressure_bar = genHistory({ base: g.lube_oil_pressure_bar, jitter: 0.05, points: 60, min: 0, now })
  ...
  if (g.vibration_level_mm_s != null)
    hist.vibration_level_mm_s = genHistory({ base: g.vibration_level_mm_s, jitter: 0.08, points: 60, min: 0, now })
  return hist
}
...
function GeneratorCard({ g }: { g: GeneratorData }) {
  ...
  return (
    <div className="rounded-(--radius-lg) border bg-(--color-card) p-4 flex flex-col gap-3">
      ...
      {!g.online ? (
        <div className="text-sm text-(--color-muted-foreground)">Unit tidak aktif</div>
      ) : (
        <>
          {typeof g.load_kw === "number" ? (
            <ProgressMeter label="Load" value={g.load_kw} unit="kW" max={2600} warnAbove={2200} critAbove={2400} />
          ) : null}
          ...
          {hist.load_kw ? (
            <div>
              <h4 className="text-xs font-medium text-(--color-muted-foreground) mb-1">Riwayat (terakhir 1 jam)</h4>
...
export default function GeneratorsSection() {
  ...
  return (
    <section aria-labelledby="generators-title" className="max-w-7xl mx-auto">
